...
import { setCachedUrl, clearCachedUrl, DEFAULT_URL } from '@/constants/config';
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  // If no custom URL saved, DEFAULT_URL is used (still counts as "set")
  useEffect(() => {
    (async () => {
      ...
      if (customUrl) {
        ...
        // No custom URL saved — use DEFAULT_URL
        // Still mark as "set" so the app flow continues (setup-url is optional if default works)
        setCachedUrl(DEFAULT_URL);
        setApiUrlState(DEFAULT_URL);

        // Verify default URL is reachable — if yes, mark as set
        try {
          const res = await fetch(`${DEFAULT_URL}/api/ping`);
          if (res.ok) {
            setIsUrlSet(true);
          } else {
            setIsUrlSet(false); // Default URL not working — user must configure
          }
        } catch {
          setIsUrlSet(false); // No internet — let user configure manually
        }
      }
    })();
  }, []);
  ...
  async function setApiUrl(url: string) {
    ...
    await StorageService.saveCustomApiUrl(trimmed);
    setCachedUrl(trimmed);
    setApiUrlState(trimmed);
    setIsUrlSet(true);
  }

  return (
    <AuthContext.Provider value={{ user, token, isLoading, login, logout, refreshUser, updatePhone, distributorPhone, fetchDistributorPhone: fetchDistPhone, companies, selectedCompanyId, setSelectedCompanyId, apiUrl, setApiUrl, isUrlSet }}>
      {children}
