...
export const ApiService = {
  ...
  // ── SMS Tracking: Log SMS/WhatsApp activity to server ──────────
  // Called after SMS is sent, skipped, or failed.
  // OFFLINE-SAFE: If network fails, saves log to local storage (offline queue).
  // The log will be synced to server when OB comes online (via syncUpload).
  logSms: async (payload: {
    shopId: string;
    shopName: string;
    shopPhone: string;
    orderbookerId: string;
    ...
    try {
      const result = await request<{ success: boolean; id: string }>('/api/sms/log', {
        method: 'POST',
        body: JSON.stringify(payload),
      });
      console.log('[ApiService.logSms] Logged to server:', result.id);
      return result;
    } catch (err: any) {
      // Network failed — save to offline queue so it syncs later
      console.warn('[ApiService.logSms] Network failed, saving to offline queue:', err?.message);
      try {
        const { StorageService } = await import('./storage');
        // Generate STABLE ID based on shopId + OB + method + status + sentAt
        const sentAtStr = new Date().toISOString();
        const stableId = `smslog_${payload.shopId}_${payload.orderbookerId}_${payload.method}_${payload.status}_${sentAtStr.substring(0, 16)}`;

        await StorageService.addOfflineSmsLog({
          id: stableId,
          shopId: payload.shopId,
          shopName: payload.shopName,
          shopPhone: payload.shopPhone,
          orderbookerId: payload.orderbookerId,
        ...
        console.log('[ApiService.logSms] Saved to offline SMS log queue (stable ID:', stableId + ')');
      } catch (storageErr) {
        console.error('[ApiService.logSms] Failed to save to offline queue:', storageErr);
      }
      return { success: false, id: '' };
    }
  },

  // ── Credit Target: Get OB's credit closing target + live stats ──
  getCreditTarget: (orderbookerId: string, month?: string) => {
    const m = month || (() => {
      const now = new Date();
