...
/**
 * Sync pending offline SMS logs to server.
 * Called automatically after recovery sync + manually during sync upload.
 * Non-blocking — errors are logged but don't affect the main sync flow.
 */
export async function syncOfflineSmsLogs(): Promise<{ synced: number; failed: number }> {
  try {
    const pendingLogs = await StorageService.getOfflineSmsLogs();
    if (pendingLogs.length === 0) {
      return { synced: 0, failed: 0 };
    }

    console.log(`[OfflineSync] Syncing ${pendingLogs.length} pending SMS logs...`);
    ...
    for (const log of pendingLogs) {
      try {
        ...
        console.warn(`[OfflineSync] SMS log sync failed for ${log.id}:`, e?.message);
        failedCount++;
      }
    }
    ...
    if (syncedIds.length > 0) {
      ...
      console.log(`[OfflineSync] Synced ${syncedIds.length} SMS logs to server`);
    }

    return { synced: syncedIds.length, failed: failedCount };
  } catch (error) {
    console.error('[OfflineSync] SMS log sync error:', error);
    return { synced: 0, failed: 0 };
  }
}
