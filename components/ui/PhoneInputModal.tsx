...
export function PhoneInputModal({ visible, shop, onPhoneSaved, onSkip }: PhoneInputModalProps) {
  ...
  const validatePhone = (p: string): boolean => {
    ...
    // Pakistan mobile formats (all accepted):
    //   3XXXXXXXXX          (10 digits, no prefix — most common, since +92 is shown in UI)
    //   03XXXXXXXXX         (11 digits, with leading 0)
    //   923XXXXXXXXX        (12 digits, with 92)
  ...
  const formatPhoneDisplay = (p: string): string => {
    ...
    // If user typed leading 0 (e.g. 0319...), strip it because +92 is already shown as prefix
    // and maxLength is 10 — keeping the 0 would block the last digit.
    if (cleaned.startsWith('0')) {
      cleaned = cleaned.substring(1);
    }
    // Hard cap at 10 digits (3XXXXXXXXX format shown next to +92)
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (normalized.startsWith('+92')) {
        ...
        // already in 03XX format
      } else {
        // Plain 3XXXXXXXXX (10 digits, no prefix) — add leading 0
