...
export const OfflineBanner = memo(function OfflineBanner({
  ...
  // Shows status only — user should use "Sync Upload" button in Profile or End Route button
  if (queueCount > 0) {
    return (
      <Animated.View style={[styles.banner, styles.pendingBanner, { opacity: fadeAnim }]}>
        ...
        {onSync ? (
          <Pressable
            ...
            hitSlop={8}
          >
            <Text style={styles.syncNowText}>Upload</Text>
          </Pressable>
        ) : null}
      </Animated.View>
    );
    ...
    return (
      <Animated.View style={[styles.banner, styles.errorBanner, { opacity: fadeAnim }]}>
        ...
        <View style={styles.textBlock}>
          ...
          <Text style={styles.bannerSub}>Use Profile → Sync Upload to retry</Text>
        </View>
        {onSync ? (
          <Pressable
            style={({ pressed }) => [styles.syncNowBtn, pressed && { opacity: 0.75 }]}
