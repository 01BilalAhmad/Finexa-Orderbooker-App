...
// Route tracking for End Route + Sync Upload button
...
export default function TodayRouteScreen() {
  ...
  const { isTracking, isStopping, endRoute, startTime } = useRouteTracking();

  // ── End Route handler (double confirmation + sync upload + redirect to route-summary) ──
  // Two-step confirmation guards against accidental "End Route" taps:
  //   Step 1: "Are you sure?" (Cancel / Yes, End)
  //   Step 2: "This action cannot be undone. Resume kar sakte hain same din." (Cancel / End & Upload)
  // After route end + sync upload, navigate to /route-summary so the user sees the
  // recovery summary and can choose Resume or Done.
  const [isEndingRoute, setIsEndingRoute] = useState(false);
  const handleEndRoute = useCallback(async () => {
    if (isStopping || isEndingRoute) return;

    Alert.alert(
      'End Route?',
      'Kya aap really aaj ka route end karna chahte hain? GPS tracking band ho jayega.',
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: () => {
            ...
            Alert.alert(
              'Final Confirmation',
              'Route end hone ke baad data sync upload hoga. Agar galti se end kiya hai to "Resume Route" button se wapas active kar sakte hain (sirf aaj). Continue?',
              [
                { text: 'Cancel', style: 'cancel' },
                {
                  text: 'End & Upload',
                  ...
                  onPress: async () => {
                    ...
                    try {
                      await endRoute();
                      // Trigger sync upload (non-fatal if fails — user can retry in Profile)
                      try {
                        const { performSyncUpload } = await import('@/services/syncUpload');
                        const result = await performSyncUpload();
                        // Auto-redirect to route-summary — no alert, no "View Summary" button
                        console.log(`[EndRoute] Sync complete: GPS=${result.locationsUploaded}, Txns=${result.transactionsSynced}`);
                        router.push('/route-summary');
                      } catch (syncErr: any) {
                        // Sync failed but route ended — still redirect to summary
                        console.warn('[EndRoute] Sync failed:', syncErr.message);
                        router.push('/route-summary');
                      }
                    } catch (e: any) {
                      Alert.alert('Error', e.message || 'Failed to end route');
                    } finally {
  ...
  }, [endRoute, isStopping, isEndingRoute]);
  ...
  const handleSync = async () => {
    // Full sync upload — GPS waypoints + route session + transactions + visits
    Alert.alert(
      'Sync Upload',
  ...
  const handleSubmitRecovery = async (payload: {
    ...
    try {
      if (isOnline) {
        ...
        // Save visit GPS coordinates locally so the map can show visited shops
        if (payload.markGpsVisit && payload.gpsLat && payload.gpsLng) {
          StorageService.saveVisitGpsCoord(shopId, payload.gpsLat, payload.gpsLng);
        }
  ...
  const handleGpsVisitMarked = async (shopId: string, gpsLat: number, gpsLng: number, address: string) => {
    ...
    StorageService.addVisitedShop(shopId);
    // Save visit GPS coordinates locally so the map can show visited shops
    StorageService.saveVisitGpsCoord(shopId, gpsLat, gpsLng);
            ...
            <View>
              ...
              >
                ...
                <View style={styles.heroTop}>
                  ...
                  {/* Route Status Indicator + End Route Button */}
                  {isTracking ? (
                    <Pressable
                      style={({ pressed }) => [
                        styles.endRouteBtn,
                        pressed && styles.endRouteBtnPressed,
                      ]}
                      onPress={handleEndRoute}
                      disabled={isStopping || isEndingRoute}
                    >
                      <View style={styles.endRouteDot} />
                      <Text style={styles.endRouteText}>
                        {isStopping || isEndingRoute ? 'Ending...' : 'End Route'}
                      </Text>
                    </Pressable>
                  ) : null}
                </View>
